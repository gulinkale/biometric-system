...
export function initEnroll() {
  ...
  const storageUsername = (localStorage.getItem("bankUsername") || "").trim();
  const storageRole = (localStorage.getItem("bankRole") || "").trim();
  ...
  const isLoggedIn = localStorage.getItem("bankLoggedIn") === "true";
  ...
  if (!storageUsername && queryUsername) {
    localStorage.setItem("bankUsername", queryUsername);
  }
  if (!storageRole && queryRole) {
    localStorage.setItem("bankRole", queryRole);
  }
